...
class Account extends BaseController
{
    ...
    public function shopSalesAccount()
    {
        ...
        $order_money=0;
        $ke=Db::table("ns_ke")->where($condition)->select();
        foreach ($ke as $key => $value) {
            $order_money+=$value['num']*$value['price'];
        }

        
      
        // 下单量
        $order_num = Db::table("ns_ordernum")->where($condition)->count('id');
        // 下单商品数
        $order_goods_num = Db::table("ns_ke")->where($condition)->sum('num');
