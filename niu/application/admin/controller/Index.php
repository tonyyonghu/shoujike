...
class Index extends BaseController
{
    ...
    public function getGoodsCount()
    {
        ...
        // 全部
        $goods_count_array['all'] = Db::table('ns_ke')->sum('num');
        // // 出售中
    ...
    public function getOrderCount()
    {
        ...
        $order_count_array['all'] = Db::table('ns_ordernum')->count('id'); // 全部订单数量，排除已删除的
    ...
    public function getSalesStatistics()
    {
        ...
        $data['curr_day_money'] = Db::table('ns_ke')->where($tmp_condition)->sum('price'); // 查询今天的订单总金额
        // $tmp_condition = $condition;
        ...
        $data['yesterday_money'] = Db::table('ns_ke')->where($tmp_condition)->sum('price'); 
        $data['yesterday_goods'] = Db::table('ns_ke')->where($tmp_condition)->sum('num'); 
        // 查看本月
        // $tmp_condition = $condition;
        ...
        $data['month_money'] = Db::table('ns_ke')->where($tmp_condition)->sum('price'); 
        $data['month_goods'] = Db::table('ns_ke')->where($tmp_condition)->sum('num'); 
        return $data;
    }
    ...
    public function getOrderChartCount()
    {
        ...
        if ($type == 1) {
            ...
            for ($i = 0; $i < 24; $i ++) {
                ...
                $count=Db::table("ns_ordernum")->where(['ordertime'=>[
                        'between',
                        [
    ...
    public function getWeiXinFansChartCount()
    {
        ...
        if ($type == 1) {
            ...
            for ($i = 0; $i < 24; $i ++) {
                $date_start = date("Y-m-d H:i:s", $start + 3600 * $i);
                $date_end = date("Y-m-d H:i:s", $start + 3600 * ($i + 1));
                $count = Db::table('ns_ke')->where(['ordertime' => [
                        'between',
                        [
                            getTimeTurnTimeStamp($date_start),
                            getTimeTurnTimeStamp($date_end)
