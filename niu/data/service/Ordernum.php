...
use data\service\Ordernum\Order as OrderBusiness;
use data\service\Ordernum\OrderAccount;
use data\service\Ordernum\OrderExpress;
use data\service\Ordernum\OrderGoods;
...
class Ordernum extends BaseService implements IOrder
{
	//private $ordernum;
    function __construct()
    {
        parent::__construct();
		$this->ordernum = new NsOrdernumModel();
    }
    ...
    {
        $order_model = new NsOrdernumModel();
	...
	*
	*/
	public function addOrEditOrdernum($group_id,$ordernum, $shouhuouser, $mobile, $province,$city,$area,$address,$desc){
		...
		if($group_id == 0){
			$this->ordernum->save($data);
			$data['group_id'] = $this->order->group_id;
			hook("goodsGroupSaveSuccess", $data);
			$res = $this->ordernum->group_id;
		}else{
			$res = $this->ordernum->save($data);
    ...
    {
        $order_count = 0;
        $orderModel = new NsOrdernumModel();
        $condition = array(
            "buyer_id" => $user_id,
            "order_id" => $order_id
        );
